...
class Gate_Atten(nn.Module):
    def __init__(self, channels, alpha=0.5):
        super(Gate_Atten, self).__init__()
        self.alpha = alpha
...
class LGCNNDTI(nn.Module):
    def __init__(self,config):
        super(LGCNNDTI, self).__init__()
        ...
        self.drug_CNN = nn.Sequential(
            *[CNN_1D(drug_channel[i], drug_channel[i+1], kernel_size=drug_kernel[i]) for i in range(len(drug_channel)-1)])
        self.protein_CNN = nn.Sequential(
            *[CNN_1D(prot_channel[i], prot_channel[i+1], kernel_size=prot_kernel[i]) for i in range(len(prot_channel)-1)])
        self.fussion_att = Gate_Atten(drug_channel[-1])
    ...
    def fussion(self, drug, prot):
        ...
        x, y = self.fussion_att(drug_x, protein_x)
